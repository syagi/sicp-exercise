      ...
      (cond ((eq? message 'get) contents)
            ...
            ((eq? message 'trace-on)
             (set! trace (lambda (contents value)
                            (print (list "set register [" name "]: " contents " to " value))))
             'trace-on)
  ...
  (let ((pc (make-register 'pc))
    ...
    (let ((the-ops
              ...
              (begin
                (if instruction-trace
                    (begin
                      (display (caar insts))
                      (newline)))
                (if (check-breakpoint breakpoint)
                  (format "break! ~s: ~s" label label-number))
      ...
      (define (cancel-breakpoint label n)
        (set! breakpoint (remove-bp (cons label n) breakpoint)))
      (define (remove-bp bp bps)
        (cond ((null? bps) (error "Cannot find in breakpoint" bp))
              ((equal? bp (car bps)) (cdr bps))
              (else (cons (car bps) (remove-bp bp (cdr bps))))))
      (define (cancel-all-breakpoints)
        (set! breakpoint '()))
      (define (check-breakpoint breakpoint)
        (cond ((null? breakpoint) #f)
              ((eq? (caar breakpoint) label)
      ...
      (define (proceed)
        (let ((insts (get-contents pc)))
         ...
         (cond ((label-exp? (caar insts))
                (set! label (cdaar insts))
                (set! label-number 1))
               (else (set! instruction-count (+ 1 instruction-count))
                     (set! label-number (+ 1 label-number))
                     (instruction-trace (caar insts))))
...
(define (make-operation-exp exp machine labels operations)
  (let ((op (lookup-prim (operation-exp-op exp) operations))
        (aprocs
         (map (lambda (e)
                (if (label-exp? e)
                    (error "Cannot apply operation to label " e)
...
(define count-leaves-machine-a
  (make-machine
   (list (list '+ +) (list 'pair? pair?) (list 'null? null?)
         (list 'not not)(list 'car car) (list 'cdr cdr))
   '(start
       (assign continue (label count-leaves-done))
       (assign val (const 0))
     count-leaves-loop
       ;; ((null? tree) 0)
       (test (op null? tree) (reg tree))
       ...
       (save continue)
       (assign continue (label count-leaves-of-car))
       (save tree)
       (assign tree (op car) (reg tree)) ;(assign tree (op vector-ref) (reg the-cars) (reg tree))
     ...
     count-leaves-of-car
       ...
       (restore continue)
       ;; (count-leaves (cdr tree))
       (assign tree (op cdr) (reg tree))  ;(assign tree (op vector-ref) (reg the-cdrs) (reg tree))
       (save continue)
       (assign continue (label count-leaves-of-cdr))
       (save val)
       (goto (label count-leaves-loop))
     count-leaves-of-cdr
       ;; 実際やっているのは carの結果とcdrの結果の加算
       ;; ラベルの名前がイマイチだが思いつかず
       (assign n (reg val))
       (restore val)
       (restore continue)
  ...
  (make-machine
   ...
   '(start
     ...
     null-tree
       ...
       (goto (reg continue))
     non-pair-tree
       (assign val (op +) (reg n) (const 1)) ; aとの差分
       (goto (reg continue))
     count-leaves-of-car
       (restore tree)
       (restore continue)
