...
(define (make-stack)
  (let ((s '())
    ...
    (define (push x)
      (set! s (cons x s))
      (set! number-pushes (+ 1 number-pushes))       ;add
      (set! current-depth (+ 1 current-depth))       ;add
      (set! max-depth (max current-depth max-depth))) ;add
    (define (pop)
      (if (null? s)
          (error "Empty stack -- POP")
          (let ((top (car s)))
            (set! s (cdr s))
            (set! current-depth (- current-depth 1))  ;add            
            top)))
    (define (initialize) 
      (set! s '())
      (set! number-pushes 0) ;add
      (set! max-depth 0)     ;add
      (set! current-depth 0) ;add
      'done)
    (define (print-statistics) ;add
      (newline)
    ...
    (let ((the-ops
           (list (list 'initialize-stack
                 ...
                 (list 'print-stack-statistics
                       (lambda () (stack 'print-statistics))))) ;add
          (register-table
...
(define (last-operand? ops)
  (null? (cdr ops)))
(define (no-more-exps? seq) (null? seq)) ;;add
...
(define eceval-operations
  (list (list 'self-evaluating? self-evaluating?)
        ...
        (list 'no-more-exps? no-more-exps?) ;;add
